    ...
    <section id="exempel" className="bg-canvas py-20 text-fg md:py-28">
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        ...
        <p className="mt-4 max-w-xl text-muted">
          Klickbara sidor, inte skisser. Välj en bransch.
        </p>
        <ul className="mt-14">
          {EXAMPLES.map((ex) => (
            <li key={ex.slug} className="border-t border-line">
              <a
                href={`/${ex.slug}`}
                className="group flex min-h-16 min-w-0 items-baseline justify-between gap-4 py-5"
              >
                <span className="min-w-0 font-display text-[1.65rem] leading-tight group-hover:text-gold md:text-5xl">
                  {ex.brand}
                </span>
